...
export default function AdminPage() {
  ...
  const [typeFilter, setTypeFilter] = useState<"ALL TYPES" | "Lost" | "Found">("ALL TYPES");
  ...
  const reportStats = useMemo(() => {
    return [
      { label: "TOTAL REPORTS", value: reports.length, icon: <FileText size={18} />, iconBg: "bg-blue-600/90" },
      { label: "PENDING REVIEW", value: reports.filter((r) => r.status === "PENDING").length, icon: <Calendar size={18} />, iconBg: "bg-orange-500/90" },
      { label: "APPROVED ITEM", value: reports.filter((r) => r.status === "APPROVED").length, icon: <FileText size={18} />, iconBg: "bg-orange-500/90" },
      { label: "CLAIMED ITEM", value: reports.filter((r) => r.status === "CLAIMED").length, icon: <Package size={18} />, iconBg: "bg-green-600/90" },
      { label: "REJECTED ITEM", value: reports.filter((r) => r.status === "REJECTED").length, icon: <Package size={18} />, iconBg: "bg-red-600/90" },
    ];
  }, [reports]);
  ...
  const filteredReports = useMemo(() => {
    ...
    return reports.filter((r) => {
      ...
      const matchType = typeFilter === "ALL TYPES" ? true : r.type === typeFilter;
      const matchStatus = statusFilter === "ALL STATUS" ? true : r.status === statusFilter;
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            <div className="space-y-5">
              ...
              <div className="relative">
                ...
                <Calendar className="pointer-events-none absolute right-4 top-[48px] text-white/25" size={16} />
              </div>

              <div className="relative">
                <TextField label="Approximate Time" name="time" type="time" required />
                <Clock className="pointer-events-none absolute right-4 top-[48px] text-white/25" size={16} />
              </div>
